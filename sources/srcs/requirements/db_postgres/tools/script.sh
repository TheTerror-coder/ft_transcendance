...
then
	...
	export POSTGRES_PASSWORD=$(curl -s --cacert $VAULT_CACERT -H "Authorization: Bearer $(cat /run/secrets/secret_access_token)" https://vault_c:8200/v1/secret/data/postgres | jq -r .data.data.password)
